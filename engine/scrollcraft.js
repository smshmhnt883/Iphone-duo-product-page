...
(function (global) {
  ...
  function mount(root, opts) {
    ...
    function tick() {
      var eps = isMobile() ? 0.012 : 0.0008;
      for (var i = 0; i < playheads.length; i++) {
        var V = playheads[i];
        if (!V.ready) continue;

        // 1. Advance the mathematical playhead position smoothly on EVERY animation frame
        if (!V.live && Math.abs(V.cur - V.target) < 0.001) continue;
        V.cur += (V.target - V.cur) * (reduce ? 1 : V.lerp);

        // 2. Decoder throttling: don't pile up seeks if decoder is still resolving previous seek
        if (V.el.seeking) {
        ...
        // 3. Issue seek to the current up-to-date playhead position
        var dur = V.el.duration || 1;
        var t = clamp(V.cur, 0, 0.999) * dur;
        if (Math.abs(V.el.currentTime - t) > eps) {
          try {
            if (typeof V.el.fastSeek === 'function') {
              V.el.fastSeek(t);
            } else {
              V.el.currentTime = t;
            }
